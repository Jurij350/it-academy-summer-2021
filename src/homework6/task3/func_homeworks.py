...
def count_price(price_rub, price_cop, count):
    ...
    считает сумму товара в рублях и копейках
...
def count_elements(new_list):
    ...
    выводит элементы которые встречаются один раз.
    """
    result_list = []
    for i in new_list:
...
def cat_dog_years(human_years):
    ...
    считает возраст собаки и кошки
...
def quarter_of(month):
    ...
    номер квартала года в зависимости от месяца.
    """
    qw = 0
    if 1 <= month <= 3:
        qw = 1
