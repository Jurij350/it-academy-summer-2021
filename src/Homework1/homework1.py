...
This is the second.
'''.splitlines()
# Use itertools.grubby and bool to return groups of
...
def validate(queens__1):
    left = right = col = queens__1[-1]
    for r in reversed(queens__1[:-1]):
        left, right = left - 1, right + 1
        if r in (left, col, right):
            raise BailOut


def add_queen(queens__2):
    for iii in range(BOARD_SIZE):
        test_queens = queens__2 + [iii]
        try:
